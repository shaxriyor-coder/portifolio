...
const loginSchema = z.object({
  email: z.string().email("Yaroqsiz email"),
  ...
  const onSubmit = async (data: { email: string; password: string }) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setErrorMessage(errorData.error || "Login muvaffaq bo'lmadi")
        return
      }
      ...
      setErrorMessage("Server bilan bog'lanish muvaffaq bo'lmadi")
    }
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4 py-10">
      <div className="w-full max-w-md rounded-3xl border border-border bg-card/80 p-8 shadow-xl shadow-black/10 backdrop-blur-xl">
        <h1 className="text-2xl font-semibold mb-4">Admin panelga kirish</h1>
        <p className="text-sm text-muted-foreground mb-6">Iltimos, admin email va parol bilan tizimga kiring.</p>
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div>
            <label htmlFor="password" className="block text-sm font-medium mb-2">Parol</label>
            <input
              id="password"
              type="password"
              {...register('password')}
